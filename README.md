...
[familyeducationcenter.ru](https://familyeducationcenter.ru) | Fullstack-разработчик | Команда 5 человек
...
- Реализовал полное клиент-серверное взаимодействие: API-клиент (native fetch), кэширование и управление запросами (TanStack Query), валидация данных (zod)
...
- Спроектировал FSD-подобную архитектуру: API-клиент (native fetch) с обработкой ошибок и валидацией (zod), сторы на Zustand, маршрутизация
...
- Реализовал авторизацию через Telegram: интеграция Login Widget + code-based авторизация (6-значный код с polling)
- Реализовал логику калькулятора с динамической оценкой стоимости услуги на основе выбранных параметров
- Реализовал отображение платёжных статусов: обработка редиректов с YooKassa, polling статуса заказа, UI для success/error/pending состояний
